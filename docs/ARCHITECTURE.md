...
| 강사 | Supabase Auth | 자기 프로필·프로그램, **자기에게 배정된 회차와 그 회차 리포트**, 자기에게 온 섭외 요청·전달된 보호자 문의, 공개 Q&A, 지역 수요 **집계치** | 학생 가명코드 단위 원본 응답, 타 강사 정보, 배정되지 않은 회차 |
| 기관·학교 담당자 | Supabase Auth | **자기 기관의** 회차·학생 가명코드·응답·관심표현·동의기록, 승인된 강사 전체 | 타 기관의 모든 데이터, 보호자 문의 원본 |
...
│   │   └── qna/             # 공개 Q&A 열람·질문 작성
│   ├── (org)/               # 기관·학교 담당자 대시보드
│   ├── (instructor)/        # 강사 프로필·프로그램·배정 회차·QR 투사·리포트·섭외·문의
...
| `lecture_sessions` | org_id, **instructor_id (nullable)**, title, field, held_on, closes_at, status, entry_code | 특강 회차. 생성 주체는 기관·학교뿐이고, `instructor_id`는 **배정**이다 (ADR-015). 강사의 회차 열람 권한이 이 컬럼에서 나온다 |
...
| `recruitment_requests` | org_id, instructor_id, field, demand_count, status | 기관·학교 → 강사 섭외 |
...
테이블 19개. `inquiries`가 추가되고 `organizations.type`·`lecture_sessions.instructor_id`가 컬럼으로 들어온다.
...
강사 워크스페이스:
로그인 → 배정 회차 목록 (lecture_sessions.instructor_id = app.current_instructor_id())
