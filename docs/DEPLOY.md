...
### AI는 지금 어떻게 동작하나 — 읽고 넘어갈 것

**`ANTHROPIC_API_KEY`가 설정돼 있지 않다. 데모 배포에서는 AI 기능 7개가 시연용 AI 응답으로 동작한다 (ADR-025·026).**

- 시연 응답은 각 기능이 LLM에 보낼 입력만 읽고 LLM이 돌려줄 모양으로 문장을 만든다. 실제 LLM 응답과 **같은 숫자·이름·모집 단어 가드**를 통과한 문장만 화면에 남고, 숫자·후보·고지문은 규칙 값 그대로다.
- 화면의 source 표시는 `시연용 AI 문장입니다`, 대시보드 상단 안내에는 "AI 문장은 시연용 예시"가 적힌다.
- `/api/health`의 `ai_mode`가 `demo`면 시연 응답, `llm`이면 실제 LLM, `rule`이면 규칙 문장이다.
- **심사·기관 앞에서는 "LLM 연결 전 시연 응답"이라고 소개한다.** 키를 넣으면(2-4절) 같은 자리에 실제 생성 문장이 들어가고 `ai_mode`가 `llm`으로 바뀐다.
- 시연 응답을 끄고 규칙 문장만 보려면 Vercel 환경변수에 `DEMO_AI=off`를 넣는다.
- 실 DB(Supabase)에 붙은 배포에서는 키가 없어도 시연 응답이 **켜지지 않는다** — 규칙 문장이 나온다.
...
| 학생 추천 (학생) | `/login` → 학생 · 계정 없음 (= `/s/735104`) → 참여 코드 `311009` 입력(지난 드론 기록이 이어진다) 또는 `코드 없이 참여하기` → 설문 제출 |
...
| 보호자 문의 도우미 (보호자) | `/login` → 보호자 · 계정 없음 (= `/inquiry`) → `예시 문장 넣기` → `문의 내용 정리하기` → `이 내용으로 폼 채우기` |

> 학생·보호자는 계정이 없으므로 역할 버튼이 아니라 **비로그인 입구**로 들어간다 (ADR-026). 학생·공개 화면 상단의 `역할 다시 고르기`로 `/login`에 돌아온다.
...
| `/login` | 역할 버튼 4개 + 계정 없이 쓰는 사람(학생·보호자) 입구 2개 |
| `/programs` | 프로그램 목록 |
