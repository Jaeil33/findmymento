...
**`ANTHROPIC_API_KEY`가 설정돼 있지 않다. 그래서 AI 기능 6개 전부가 규칙 결과로만 동작한다.**

학생 화면에 뜨는 추천 이유는 LLM이 만든 문장이 아니라 `ruleReason()`이 만든 템플릿 문장이다. 회차 기획·교안 초안·결과보고서·후속 과정 제안·수업 회고도 같다 — 전부 규칙이 만든 초안이 나온다. 설계상 의도된 동작이고(E-06, ADR-004·017·021·024) 화면은 정상으로 보이지만, **지금 이 데모에서 AI는 한 줄도 돌지 않는다.**

- 기관·심사 자리에서 이 화면들을 "AI 기능"으로 소개하려면 **키를 먼저 넣어야 한다** (2-4절).
